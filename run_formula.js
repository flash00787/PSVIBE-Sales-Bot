...
print("=" * 92)
print(f"{'Account':12s} {'Income':>10s} {'OPEX':>10s} {'TxIn':>10s} {'TxOut':>10s} {'Inj':>10s} {'Ej':>10s} {'CapEx':>10s} {'= BAL':>12s}")
print("-" * 92)
...
for a in accounts:
    ...
    o = opex_by_acct[k]
    ti = cash_map.get(f"transfer_in|{lbl}", 0)
    to = abs(cash_map.get(f"transfer_out|{lbl}", 0))
    ij = cash_map.get(f"inject|{lbl}", 0)
    ej = cash_map.get(f"eject|{lbl}", 0)
    capex = capex_total if k == "kbz_bank" else 0
    bal = inc - o + ti - to + ij - ej - capex
    results.append((lbl, inc, o, ti, to, ij, ej, capex, bal))
    print(f"{lbl:12s} {inc:>10,.0f} {o:>10,.0f} {ti:>10,.0f} {to:>10,.0f} {ij:>10,.0f} {ej:>10,.0f} {capex:>10,.0f} {bal:>+12,.0f}")

total_bal = sum(r[8] for r in results)
print("-" * 92)
print(f"{'TOTAL':12s} {'':>10s} {'':>10s} {'':>10s} {'':>10s} {'':>10s} {'':>10s} {'':>10s} {total_bal:>+12,.0f}")
print()
...
cur_accounts = {r["account_name"]: float(r["balance"]) for r in cur.fetchall()}
print(f"{'Account':12s} {'Current':>12s} {'Formula':>12s} {'Diff':>12s}")
print("-" * 48)
for lbl, inc, o, ti, to, ij, ej, capex, bal in results:
    current = cur_accounts.get(lbl, 0)
    print(f"{lbl:12s} {current:>12,.0f} {bal:>12,.0f} {bal-current:>+12,.0f}")
...
conn.on('ready', () => {
  conn.exec('python3 /dev/stdin', (err, stream) => {
    ...
    stream.stderr.on('data', d => { 
      if (!d.toString().includes('DeprecationWarning')) 
