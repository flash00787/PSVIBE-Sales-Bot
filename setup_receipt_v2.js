...
conn.on('ready', () => {
  const commands = `
    ...
    mkdir -p "$API_DIR"
    # Also create root-level receipts dir for template (per task spec)
    mkdir -p "$BOT_DIR/receipts"
...
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .promo-badge { display: inline-block; background: var(--success); color: #000; font-size: 9px; font-weight: 700; padding: 1px 6px; border-radius: 4px; margin-left: 4px; vertical-align: middle; }
...
Routes:
  ...
  POST /api/z2a3b4               → Receive receipt JSON (requires x-receipt-secret)
...
RECEIPTS_DIR = BOT_DIR / "bot" / "receipts"
TEMPLATE_PATHS = [
    BOT_DIR / "receipts" / "template.html",       # task spec location
    BOT_DIR / "bot" / "receipts" / "template.html",  # legacy location
]
    ...
    if _TEMPLATE_CACHE is None:
        for p in TEMPLATE_PATHS:
            if p.exists():
                ...
                logger.info("Loaded receipt template from %s", p)
                break
        if _TEMPLATE_CACHE is None:
            logger.warning("Receipt template not found (checked: %s)", TEMPLATE_PATHS)
...
async def serve_receipt(voucher_id: str, request: Request):
    """Serve printable HTML receipt page."""
    ...
    html = html.replace("</head>", f"<script>window.__RECEIPT_DATA__ = {data_json};</script></head>", 1)
...
async def receive_receipt(
    ...
    """Receive receipt JSON from bot and persist locally."""
...
PYEOF
    ...
    echo ""
    echo "=== Step 4: Create api_server/__init__.py ==="
...
PYEOF
    ...
    echo ""
    echo "=== Step 5: Create api_server/main.py (FastAPI application) ==="
    
    cat > "$API_DIR/main.py" << 'PYEOF'
"""
PS VIBE V2 — FastAPI API Server
Runs at localhost:8000. Routes:

  /api/z2a3b4/*     → Receipt handlers (GET/POST)
  /api/health       → Health check
  /api/*            → Other handlers (added by api_setup_agent)

All routes use obfuscated paths per path_mapping.json.
"""
import os
import sys
import logging
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler("/tmp/psvibe_api_server.log", encoding="utf-8"),
    ],
)
logger = logging.getLogger("psvibe_api")


# ── Import and include receipt handler ──────────────────────────────────
try:
...
PYEOF
    ...
    echo "RECEIPT_SECRET value: $RECEIPT_SECRET"
    
    echo ""
    echo "=== Step 7: Install FastAPI/uvicorn if missing ==="
    pip3 install fastapi uvicorn python-multipart 2>&1 | tail -2 || echo "pip install check done"
    
    echo ""
    echo "=== Step 8: Create test receipt ==="
...
TESTEOF
    ...
    echo ""
    echo "=== Step 9: Install and configure Caddy reverse proxy ==="
...
# PS VIBE V2 — Caddy Reverse Proxy
...
CADDYEOF
    ...
    echo ""
    echo "=== Step 11: Test receipt endpoint ==="
    curl -s http://localhost:8000/api/z2a3b4/test-voucher | head -20
    echo ""
    echo "---"
    curl -s -o /dev/null -w "HTTP %{http_code}" http://localhost:8000/api/z2a3b4/test-voucher
    echo ""
