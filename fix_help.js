...
const helpFixed = `"""PS VIBE Bot — Help, Version & Error Handler.
"""
...
conn.on('ready', () => {
  conn.exec('cat > /root/staging/bot_src/bot/handlers/help.py', (err, stream) => {
    stream.write(helpFixed);
    stream.end();
    let out = '';
    stream.on('data', d => out += d);
    stream.stderr.on('data', d => out += d);
    stream.on('close', () => { console.log(out || 'help.py written'); conn.end(); process.exit(0); });
  });
}).connect({
  host: '167.71.196.120',
  username: 'root',
