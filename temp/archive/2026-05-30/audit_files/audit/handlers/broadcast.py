...
import logging, re, json, html, asyncio
...
async def cmd_broadcast(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Escape HTML special chars and cap at 1000 chars to prevent abuse
    safe_text = html.escape(msg_text)[:1000]
    full_text = f"📢 <b>PS Vibe</b>\n\n{safe_text}\n\n—\n<i>Unsubscribe /stop</i>"
    ...
    for tg_id in telegram_ids:
        try:
            ...
            sent += 1
            await asyncio.sleep(0.1)   # max ~10 msg/sec — well within Telegram rate limits
        except Exception as e:
            logging.warning("Broadcast failed for %s: %s", tg_id, e)
            failed += 1

    logging.info("Broadcast complete: sent=%d, failed=%d, total_targets=%d", sent, failed, len(telegram_ids))
