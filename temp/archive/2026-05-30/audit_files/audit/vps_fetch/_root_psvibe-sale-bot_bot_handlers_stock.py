...
async def step_stock_pin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        logging.exception("HANDLER_ERROR: delete_stock_pin_msg")
        # Fallback: try to edit the PIN message to hide it
        try:
            await update.message.edit_text("•••• (PIN ေဖျောက်ပြီး)")
        except Exception:
            logging.exception("HANDLER_ERROR: edit_stock_pin_msg")
            # Last resort: notify user to delete manually
            try:
                await update.message.reply_text(
                    "⚠️ လုံခြုံရေးအတွက် PIN မက်ဆေ့ချ်ျကို ကိုယ်တိုင်ဖျက်ပါ။",
                )
            except Exception:
                logging.exception("HANDLER_ERROR: reply_stock_pin_sec_warn")
