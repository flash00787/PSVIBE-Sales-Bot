...
# ── Staff Bot Whitelist ──────────────────────────────────────────────────────
ALLOWED_USER_IDS = _bot_module.ALLOWED_USER_IDS
...
async def show_main_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Quick-access inline Home button (always visible)
    home_kb = InlineKeyboardMarkup([[
        InlineKeyboardButton("\U0001f3e0 Home / Refresh Menu", callback_data="home:refresh"),
    ]])
    await update.message.reply_text(
        "\u23f0 _Quick access:_",
        parse_mode="Markdown",
...
# ════════════════════════════════════════
#  INLINE HOME BUTTON HANDLER
# ════════════════════════════════════════

async def cb_home(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Inline button handler — return to main menu from any state."""
    query = update.callback_query
    await query.answer()
    context.user_data.clear()
    await query.delete_message() if query.message else None
    return await show_main_menu(update, context)
...
async def _PLACEHOLDER_prompt_discount(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """PLACEHOLDER — replaced by new prompt_discount above."""
    d     = context.user_data
...
# ══════════════════════════════════════════════════════════════════════════════
#  PROMO REPORTS  (Promotions sheet + Sales data)
# ══════════════════════════════════════════════════════════════════════════════
...
_SESSION_END_TIMES: dict[str, str] = {}   # key -> "HH:MM" current planned end
