...
_ALLOWED_USER_IDS: set[int] = {8539344655, 8336350778, 6296803251}
        ...
        def _do():
            ...
            topup_sh.batch_update(
                [{"range": f"A{tl_row}:C{tl_row}",
                 ...
                 {"range": f"E{tl_row}:M{tl_row}",
                  "values": [[nm_amt, nm_kpay, nm_cash, nm_mins, tl_type, 0, 0, 0]]},
        ...
        def _do():
            topup_sh.batch_update(
                [{"range": f"A{tl_row}:C{tl_row}",
                 ...
                 {"range": f"E{tl_row}:M{tl_row}",
                  "values": [[tu_amt, tu_kpay, tu_cash, tu_mins, "Top Up", 0, 0, 0]]}],
                value_input_option="USER_ENTERED",
            )
            if new_rate > 0:
    ...
    async def _sale_bg():
        def _do():
            sales_sh.batch_update(
                [{"range": f"A{s_row}:R{s_row}",
                  "values": [[today, v_no, m_id, c_id, play_mins,
                              game_amt, food_total, _disc, net_total, kpay, cash, 0, 0, 0]]},
...
topup_wave=0,        topup_cb=0,        topup_aya=0,        sales_wave=0,        sales_cb=0,        sales_aya=0,
    ...
    try:
        for row in topup_sh.get_all_values()[1:]:
            ...
            if not d or d.year != year_i or d.month != mon_i:
                continue
            res["topup_wave"] += _int(row[10]) if len(row) > 10 else 0
            res["topup_cb"]   += _int(row[11]) if len(row) > 11 else 0
            res["topup_aya"]  += _int(row[12]) if len(row) > 12 else 0
...
def _replit_post(path: str, body: dict, timeout: int = 8):
...
async def _post_n8n_session_reminder(
    ...
    try:
        ...
        await asyncio.to_thread(lambda: _req.urlopen(request, timeout=6))
        return True
    except Exception as e:
...
async def _post_n8n_booking_reminder(
    ...
    try:
        ...
        await asyncio.to_thread(lambda: _req2.urlopen(req, timeout=6))
