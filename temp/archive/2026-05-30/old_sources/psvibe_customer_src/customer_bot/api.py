...
async def _fetch_members() -> dict:
    ...
    if cached is not None:
        return cached
    data    = await _api_get("sheets/members-list")
...
async def _fetch_consoles() -> list:
    ...
    if cached is not None:
        return cached
    data     = await _api_get("sheets/consoles")
...
async def _fetch_promotions() -> list[dict]:
    ...
    if cached is not None:
        return cached
    data = await _api_get("sheets/promotions")
...
async def _fetch_sales_data() -> dict:
    ...
    if cached is not None:
        return cached
    try:
        data = await _api_get("sheets/sales-summary")
        if data:
