...
from telegram import filters as _ptb_filters
...
def _build_ai_system_prompt(
    ...
    """
    config = fetch_config_fn() if fetch_config_fn else {}
    ...
    rate_lines = build_rate_lines_fn() if build_rate_lines_fn else []
    if rate_lines:
        rates_text = "\n".join(rate_lines)
    elif base_rate:
    ...
    bonus_table_text = build_bonus_table_fn(config) if build_bonus_table_fn else ""
    ...
    if build_live_game_library_fn and fetch_games_full_fn:
        live_library = build_live_game_library_fn(fetch_games_full_fn)
