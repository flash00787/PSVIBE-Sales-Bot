...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    today_bks = await _api._api_get(f"bookings?telegramChatId={uid}&date={today_mmt()}&status=confirmed")
...
async def cmd_today(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    consoles, bks = await asyncio.gather(
        ...
        _api._api_get(f"bookings?date={today}&status=confirmed"),
    )
...
async def cmd_console_status(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    consoles, today_bks = await asyncio.gather(
        _api._fetch_consoles(),
        _api._api_get(f"bookings?date={today_mmt()}&status=confirmed"),
    )
...
async def cb_feedback_rating(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if rating >= 4:
        asyncio.create_task(_api._api_post("feedback", {
