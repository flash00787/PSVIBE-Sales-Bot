#!/usr/bin/env python3
"""Generate DividendsView.vue and upload to VPS."""
...
const summary = ref<{ shareholders: SummaryShareholder[]; total_paid: number } | null>(null)
...
async function fetchDividends() {
  ...
  try {
    ...
    if (r.data.success) {
      dividends.value = r.data.data || []
    } else {
      error.value = r.data.error || 'Failed to load'
    }
  } catch (e: any) {
    error.value = e.response?.data?.error || e.message || 'Network error'
  } finally {
    loading.value = false
  }
}
...
async function submitDividend() {
  if (!form.value.shareholder_id || form.value.amount <= 0) {
    error.value = 'Please select a shareholder and enter a valid amount'
    return
  }
  ...
  try {
    ...
    if (r.data.success) {
      ...
      await Promise.all([fetchDividends(), fetchSummary()])
    } else {
      error.value = r.data.error || 'Failed to record'
    }
  } catch (e: any) {
    error.value = e.response?.data?.error || e.message || 'Network error'
  } finally {
...
onMounted(async () => {
  await Promise.all([fetchDividends(), fetchSummary(), fetchShareholders()])
})
</script>
  ...
  <AppLayout>
    <div class="p-4 lg:p-6">
      <div class="mb-6 flex flex-wrap items-center gap-4">
        ...
        <button
          @click="openModal"
          class="px-4 py-2 bg-vibe-purple text-white rounded-lg text-sm hover:bg-purple-700 transition"
        >
          + Record Dividend
        </button>
      </div>
      ...
      <div v-if="summary" class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        ...
        <div
          v-for="sh in summary.shareholders"
          ...
          <tbody class="divide-y divide-gray-100">
            <tr v-for="d in dividends" :key="d.id" class="hover:bg-gray-50">
              ...
              <td class="px-4 py-3 text-center">
                <span :class="statusClass(d.status)" class="inline-block px-2 py-0.5 rounded-full text-xs font-medium">
                  {{ d.status }}
                </span>
              </td>
        ...
        <div class="bg-white rounded-2xl shadow-xl w-full max-w-md mx-4 p-6">
          ...
          <div class="space-y-4">
            ...
            <div>
              ...
              <select
                ...
                class="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-vibe-purple"
              >
                <option v-for="sh in shareholders" :key="sh.id" :value="sh.id">
                  {{ sh.name }}
                </option>
              </select>
            </div>
          ...
          <div class="flex justify-end gap-3 mt-6">
            ...
            <button
              ...
              class="px-4 py-2 text-sm bg-vibe-purple text-white rounded-lg hover:bg-purple-700 disabled:opacity-50"
            >
              {{ formSubmitting ? 'Saving...' : 'Save Dividend' }}
            </button>
          </div>
        </div>
      </div>
...
# Write to VPS using Python heredoc via vps_exec
script = f"""
cat > /root/psvibe-dashboard/src/views/DividendsView.vue << 'VUEEOF'
{content}
VUEEOF
echo "DividendsView.vue written OK"
"""
subprocess.run(['node', '/home/node/.openclaw/workspace/coordination/vps_exec.js', script], check=True)
print("DividendsView.vue uploaded to VPS")
