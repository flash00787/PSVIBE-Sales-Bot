...
async def verify_api_key(
    ...
    """Accept API key via query param (?api_key=...) or X-API-Key header."""
    key = api_key
...
async def api_sheets_config(auth=Depends(verify_api_key)):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=str(e))
# ═══════════════════════════════════════
#  MYSQL — Member & Console Endpoints
# ═══════════════════════════════════════
@app.get("/api/mysql/members", tags=["MySQL"])
async def mysql_fetch_members(api_key: str = Depends(verify_api_key)):
    """Fetch all member IDs from MySQL (faster than gspread)."""
    ...
    try:
        ...
        for r in rows:
            if r.get("member_id"):
                members.append(r["member_id"])
        return ok(members)
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
@app.get("/api/mysql/member_data/{member_id}", tags=["MySQL"])
async def mysql_fetch_member_data(member_id: str, api_key: str = Depends(verify_api_key)):
    """Fetch member data from MySQL."""
    if not _mysql_available:
        raise HTTPException(status_code=503, detail="MySQL not available")
    from db_client import DatabaseManager
    db = DatabaseManager()
    try:
        ...
        raise HTTPException(status_code=500, detail=str(e))
@app.get("/api/mysql/console_status", tags=["MySQL"])
async def mysql_fetch_console_status(api_key: str = Depends(verify_api_key)):
    """Fetch console status from MySQL."""
    if not _mysql_available:
        raise HTTPException(status_code=503, detail="MySQL not available")
    from db_client import DatabaseManager
    db = DatabaseManager()
    try:
        consoles = db.get_console_status()
        return ok(consoles if consoles else [])
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
@app.get("/api/mysql/games", tags=["MySQL"])
async def mysql_fetch_games(api_key: str = Depends(verify_api_key)):
    """Fetch game library from MySQL."""
    if not _mysql_available:
        raise HTTPException(status_code=503, detail="MySQL not available")
    from db_client import DatabaseManager
    db = DatabaseManager()
    try:
        games = db.get_games()
        return ok(games if games else [])
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
@app.get("/api/mysql/console_games", tags=["MySQL"])
async def mysql_fetch_console_games(api_key: str = Depends(verify_api_key)):
    """Fetch console-game installations from MySQL."""
    if not _mysql_available:
        raise HTTPException(status_code=503, detail="MySQL not available")
    from db_client import DatabaseManager
    db = DatabaseManager()
    try:
        rows = db._fetch_all("SELECT * FROM console_games")
        return ok(rows if rows else [])
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
@app.get("/api/mysql/staff", tags=["MySQL"])
async def mysql_fetch_staff(api_key: str = Depends(verify_api_key)):
    """Fetch staff records from MySQL."""
    if not _mysql_available:
        raise HTTPException(status_code=503, detail="MySQL not available")
    from db_client import DatabaseManager
    db = DatabaseManager()
    try:
        staff = db.get_staff(active_only=False)
        return ok(staff if staff else [])
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
# ═══════════════════════════════════════
#  MYSQL — Sync & Health Endpoints
# ═══════════════════════════════════════
@app.post("/api/mysql/sync", tags=["MySQL"])
async def manual_sync(api_key: str = Depends(verify_api_key)):
    """Trigger a one-time sync from Google Sheets to MySQL."""
    if not _mysql_available:
        raise HTTPException(status_code=503, detail="MySQL not available")
    ...
    try:
        svc = SyncService()
        result = svc.sync_all()
        return ok({"synced": True, "details": str(result)})
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
@app.get("/api/mysql/sync_status", tags=["MySQL"])
async def sync_status(api_key: str = Depends(verify_api_key)):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=str(e))
@app.get("/api/mysql/health", tags=["MySQL"])
async def mysql_health(api_key: str = Depends(verify_api_key)):
    ...
    try:
        ...
        sync_running = (
            _sync_service is not None
            and _sync_service._bg_thread is not None
            and _sync_service._bg_thread.is_alive()
        )
        return ok({"mysql": healthy, "sync_running": sync_running})
    except Exception as e:
        return ok({"mysql": False, "error": str(e)})
# ═══════════════════════════════════════
...
async def api_analytics_weekly_trends(
    ...
    try:
        from analytics import get_topup_trends, get_console_usage, get_daily_sales, today_str
        days = weeks * 7
...
function render(d) {
  const s = d.summary, sales = d.daily_sales, mem = d.member_activity, con = d.console_usage_today, tup = d.topup_trends_7d;
  let h = '';
  // KPI row
  ...
  // Row: Sales detail + Member activity
  h += '<div class="row">';
...
async def web_dashboard():
    ...
    html = DASHBOARD_HTML.replace(
        ...
        f"fetch('/api/analytics/dashboard?api_key={API_KEY}')"
    )
...
async def startup_mysql():
    """Initialise MySQL connection pool and start background sync (optional)."""
    global _mysql_available, _sync_service
    if not MYSQL_HOST:
        logger.info("MySQL not configured — running in gspread-only mode")
        return
    try:
        from db_client import DatabaseManager
        db = DatabaseManager()
        ...
        logger.info("MySQL connected: %s:%s/%s", MYSQL_HOST, MYSQL_PORT, MYSQL_DATABASE)

        # Start background sync
        from sync_service import SyncService
        _sync_service = SyncService()
        _sync_service.start_background_sync(interval_seconds=SYNC_INTERVAL)
        logger.info("Background sync started (interval=%ds)", SYNC_INTERVAL)
    except Exception as e:
        logger.warning("MySQL unavailable — falling back to gspread: %s", e)
        _mysql_available = False

@app.on_event("shutdown")
async def shutdown_mysql():
    """Cleanly stop background sync on shutdown."""
