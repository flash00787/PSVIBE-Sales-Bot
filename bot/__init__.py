...
async def fetch_console_status_async() -> list[dict]:
    ...
    for c in result.get("consoles", []):
        mapped.append({
            ...
            "startTimeDt": c.get("start_time_dt"),
            "staff":     c.get("staff_name"),
            "booking_id": c.get("booking_id"),
            "duration_mins": c.get("duration_mins", 0),
        })
    return mapped
...
# ── fetch_alltime_effective_rate (moved from admin.py) ──
...
class BotState(IntEnum):
    ...
    SAL_ADV_PAY = 46
    SAL_ADV_CONFIRM = 47
    BOOK_LINK = 48  # DEPRECATED — removed from Session Start flow, kept for backward compat
    BOOK_CONSOLE = 49
    BOOK_MEMBER = 50
    CONSOLE_MENU = 51
    END_SESSION_SELECT = 52
    END_SESSION_CONFIRM = 193
    GAME_MENU = 53
    GAME_ADD_TITLE = 54
    ...
    SSD_MOVE_GAME = 181
    SSD_MOVE_CONS = 182
    SSD_MOVE_FROM_CONS = 183
    SSD_MOVE_FROM_GAME = 184
    SSD_MOVE_TO_SSD = 185
    DISC_SELECT = 98
    DISC_SET_QTY = 99
    FINANCE_MENU = 100
    ...
    GAME_EDIT_FIELD = 173
    GAME_EDIT_VALUE = 174
    GAME_DETAIL_PICK = 179
    ADJUST_TIME = 175
    WL_MENU = 176
    BOOK_CHECKIN_BIND = 189
...
BOOK_CHECKIN_BIND = BotState.BOOK_CHECKIN_BIND
CONSOLE_MENU = BotState.CONSOLE_MENU
END_SESSION_SELECT = BotState.END_SESSION_SELECT
END_SESSION_CONFIRM = BotState.END_SESSION_CONFIRM
...
MOVECON_SOURCE = BotState.MOVECON_SOURCE
MOVECON_TARGET = BotState.MOVECON_TARGET
MOVECON_CONFIRM = BotState.MOVECON_CONFIRM
SWAP_SOURCE = BotState.SWAP_SOURCE
SWAP_CONFIRM = BotState.SWAP_CONFIRM
SESSION_SHORTFALL = BotState.SESSION_SHORTFALL
SALE_GAME_SELECT = BotState.SALE_GAME_SELECT
...
SBK_PHONE = BotState.SBK_PHONE
SBK_DATE = BotState.SBK_DATE
SBK_TIME = BotState.SBK_TIME
SBK_DURATION = BotState.SBK_DURATION
...
GAME_DETAIL_PICK = BotState.GAME_DETAIL_PICK
...
BTN_FOOD_SALE      = "🍔 Food Sale"
BTN_FOOD_NOTE      = "🍔 Food Order"
...
STAFF_NOTIFY_THREAD  = int(os.environ.get("STAFF_NOTIFY_THREAD_ID", "125192"))  # topic/thread ID in staff group
