...
async def step_nm_amt(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Staff confirmed the default price (robust match / skip payment for default)
...
async def prompt_tu_kpay(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # Show amount input when method selected but no amount entered yet
    if d.get("tu_show_amount_input") and d.get("tu_current_pay_method"):
        ...
        await update.message.reply_text(
            f"💳 *{method}* ပမာဏ ရိုက်ပါ\n"
            f"━━━━━━━━━━━━━━━━━━\n"
            f"💰 Top Up: *{amt:,} Ks*  |  Remaining: *{remaining:,} Ks*\n"
            f"━━━━━━━━━━━━━━━━━━\n"
            f"(0 - {remaining:,}) ထည့်ပါ -",
            parse_mode="Markdown",
            reply_markup=ReplyKeyboardMarkup([NAV_ROW], resize_keyboard=True),
        )
        return TU_KPAY
