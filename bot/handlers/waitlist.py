...
from bot import (
    ...
    BTN_WL_VIEW_WAITING, MMT, WL_MENU, _psvibe_delete, _psvibe_delete_async, _psvibe_get, _psvibe_get_async,
    _psvibe_post, _psvibe_post_async, show_main_menu,
)
...
async def _show_wl_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    store = await _psvibe_get_async("waitlist")
...
async def step_wl_menu(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if choice in (BTN_WL_REFRESH, BTN_WL_VIEW_WAITING):
        store = await _psvibe_get_async("waitlist?status=waiting")
        rows = store.get("rows", []) if store else []
        if not rows:
    ...
    if choice == BTN_WL_NOTIFY_NEXT:
        ...
        wl_store = await _psvibe_get_async("waitlist?status=waiting")
        ...
        resp = await _psvibe_post_async("waitlist/notify", {})
        if not resp:
...
async def cb_wl_action(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if action == "notify":
        entry_data = await _psvibe_get_async(f"waitlist/{entry_id}")
        ...
        resp = await _psvibe_delete_async(f"waitlist/{entry_id}")
