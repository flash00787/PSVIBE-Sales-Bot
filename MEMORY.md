...
| 198 | **Dedicated notification channels reduce group spam** — `DEPOSIT_NOTIFY_CHAT` env variable routes deposit/booking notifications separately from `STAFF_NOTIFY_CHAT`. |
| 197 | **Refund 1-hour rule** — Refunds only allowed up to 1 hour before booking start. After that, deposit is forfeited as no-show penalty. Both front-end and backend enforce this. |
| 196 | **Refund ≠ Forfeited** — Three distinct deposit statuses: `paid` (submitted, pending verification), `forfeited` (no-show, shop keeps), `refunded` (money returned to customer). Each has different accounting treatment. |
| 195 | **429 Rate Limit from excessive parallel API calls** — 16 parallel `/api/bookings` calls triggered Cloudflare rate limiting. Always combine multiple status-filtered calls into one and use client-side `.filter()` grouping. |
