...
| 72 | GET | `/api/bot-users/booking-success-rate` | Bot Users | Get booking success rate stats |
...
### `GET /api/bot-users/booking-success-rate`
**Purpose:** Get Customer Bot booking success rate statistics with rebook-adjusted metrics.
**Auth required:** Yes
**Query Parameters:**

| Name | Type | Default | Description |
|------|------|---------|-------------|
| since | string | "2026-06-02" | Filter bookings from this date (YYYY-MM-DD) |

**Response Data Fields:**

| Field | Description |
|-------|-------------|
| totals | Status counts (total, done, active, cancelled_raw, cancelled_adj, rebooked, rejected, pending, confirmed) |
| success_rate_pct | Raw success rate (Done+Active)/Total |
| success_rate_adj_pct | Adjusted success rate — rebooked cancellations excluded from failure |
| daily_trend | 14-day daily breakdown (total, done, cancelled, rejected, rate) |
| weekly_trend | 4-week weekly breakdown |
| console_breakdown | Per-console stats (C-01 to C-10) |
| top_users | All customer bot users with individual stats (name, phone, tg_id, username, bookings, success rate) |

**Rebook Logic:** A cancelled booking is marked as "rebooked" if the same phone+date+game later had a Done/Active booking.
**Exclusions:** Boss test bookings (6296803251), staff group chat (-1003686032747).
