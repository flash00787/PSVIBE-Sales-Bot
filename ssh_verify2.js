...
async function runDeploy() {
  const cmds = [
    {
      label: 'Login and verify finance API',
      cmd: `curl -s -X POST http://localhost:8000/auth/login -H "Content-Type: application/json" -d '{"username":"admin","password":"admin123"}' 2>&1 | python3 -m json.tool`,
      timeout: 10000,
    },
  ];
